...
//    }
    ...
    public static void main(String[] args) {
        assert Objects.equals(strip(""), "");
        assert Objects.equals(strip("  \n  \t  "), "");
        assert Objects.equals(strip("  \n  \t  a  \n  \t  "), "a");
        System.out.println(Objects.equals(strip("  \n  \t  a b\n  \t  "), "a b"));
        System.out.println(listReconstruct("[All,All,[1,5]]").equals("[\"All\",\"All\",[1,5]]"));
        System.out.println(listReconstruct(" [ All, All, [1 , 5 ] ]").equals("[\"All\",\"All\",[1,5 ]]"));
//        System.out.println(listReconstruct(" [All  ,All, All,[1 , 5 ]] "));
        System.out.println(listReconstruct(" [ All, All, [ 1 , 5 ] ]").equals("[\"All\",\"All\",[ 1,5 ]]"));
    }
    ...
    public static String strip(String str) {
        ...
        for (int i = 0; i < str.length() && left < right; i++) {
            if (space.indexOf(str.charAt(left)) != -1) { // find left space
                left++;
            }
            if (space.indexOf(str.charAt(right)) != -1) {
                right--;
            }
        }
    ...
    /**
     * [All,All,[1,5]] -> ["All", "All", [1,5]]
    */
    private static String listReconstruct(String list) {
        String source = strip(list);
        String[] content = source.substring(1, source.length()-1).split(",");
        // "[All,All,[1,5]]" -> ["All","All","[1","5]"]
    ...
    /**
     * guess whether this value needs wrap with []
     * */
    private static String guessWrap(String value) {
        if (Objects.isNull(value) || value.isEmpty()) return "";
        String stripped = strip(value);
        if(stripped.startsWith("[") && stripped.endsWith("]")) {
            return stripped; // no need, fuck off
        } else {
            return String.format("[%s]", stripped);
        }
    }
    ...
    public static String generateVulnerabilityFileContent(Vulnerability vulnerability) {
        return "---------- Fragility ----------\n" +
                ...
                guessWrap(vulnerability.getDS_Bld()) + " -- DS_Bld -- Damage states of blades (Blade-tip deflection m) [used only when Frgl_Bld=1]\n" +
                listReconstruct(vulnerability.getIMI_Bld()) + " -- IMI_Bld -- IM intensity for distribution models estimation for blades {\"All\" or [ith IM, jth IM]} [used only when Frgl_Bld=1]\n" +
                "[" + vulnerability.getFrgl_Nac() + "] -- Frgl_Nac -- Fragility of nacelle {Switch: 1=true, 2=false}\n" +
                guessWrap(vulnerability.getDS_Nac()) + " -- DS_Nac -- Damage states of nacelle (Nacelle acceleration m/s2) [used only when Frgl_Nac=1]\n" +
                listReconstruct(vulnerability.getIMI_Nac()) + " -- IMI_Nac -- IM intensity for distribution models estimation for nacelle {\"All\" or [ith IM, jth IM]} [used only when Frgl_Nac=1]\n" +
                ...
                guessWrap(vulnerability.getDS_Twr()) + " -- DS_Twr -- Damage states of tower [used only when Frgl_Twr=1]\n" +
                listReconstruct(vulnerability.getIMI_Twr()) + " -- IMI_Twr -- IM intensity for distribution models estimation for tower {\"All\" or [ith IM, jth IM]} [used only when Frgl_Twr=1]\n" +
                ...
                guessWrap(vulnerability.getDS_Sub()) + " -- DS_Sub -- Damage states of monopile [used only when Frgl_Mop=1]\n" +
                listReconstruct(vulnerability.getIMI_Sub()) + " -- IMI_Sub -- IM intensity for distribution models estimation for monopile {\"All\" or [ith IM, jth IM]} [used only when Frgl_Mop=1]\n" +
                guessWrap(vulnerability.getHIF()) + " -- HIF -- Hazard interval for fragility output\n" +
                ...
                guessWrap(vulnerability.getLossR_Bld()) + " -- LossR_Bld -- Loss ratio definition for blades [used only when Frgl_Bld=1]\n" +
                guessWrap(vulnerability.getLossR_Nac()) + " -- LossR_Nac -- Loss ratio definition for nacelle [used only when Frgl_Nac=1]\n" +
                guessWrap(vulnerability.getLossR_Twr()) + " -- LossR_Twr -- Loss ratio definition for tower [used only when Frgl_Twr=1]\n" +
                guessWrap(vulnerability.getLossR_Mop()) + " -- LossR_Mop -- Loss ratio definition for monopile [used only when Frgl_Mop=1]\n" +
                "---------- Dependency ----------\n" +
    ...
    public static String generateConditionFileContent(Long taskId, Condition condition) {
        ...
        return "---------- Analyze type ----------\n" +
                ...
                ("1".equals(condition.getIMImpM()) ? guessWrap(condition.getWindSpd()) : String.format("[\"%s/%s\"]", getInputDir(taskId), "tmp1.inp")) + " -- WindSpd -- Wind speed at hub height (m/s), Parameter = [Cut-in ,Cut-out, Step] when IMImp=1, and input file when IMImp=2\n" +
                "[" + condition.getWPInpM() + "] -- WPInpM -- Wave parameters input method {1=still water, 2=using wind-wave correlation, 3=user-defined file}\n" +
                "[" + ("3".equals(condition.getWPInpM()) ? "" : String.format("\"%s/%s\"", getInputDir(taskId), "tmp2.inp")) + "] -- WaveHsF -- Input file for significant wave height (m) [used only when WPInpM=3]\n" +
                "[" + ("3".equals(condition.getWPInpM()) ? "" : String.format("\"%s/%s\"", getInputDir(taskId), "tmp3.inp")) + "] -- WaveTpF -- Input file for wave peak period (s) [used only when WPInp=3]\n" +
                ...
                "---------- OpenSees Seeting ----------\n" +
                (condition.getIM_Model().indexOf('[') >= 0 ? condition.getIM_Model() : String.format("['%s']", condition.getIM_Model())) + " -- IM_Model -- Model selected to perform Analyses in OpenSees {[1,2,3...] or [\"ALL\"] or [\"None\"]}\n" +
